  ...
  return (
    <footer className="fixed-bottom bg-light py-3">
      <Container className="bg-light text-muted">
        {( frontendVersion && backendInfo.data?.version?.KeinPlanBackend != frontendVersion) &&
          <Alert variant="danger" className="mb-3">
            Auf diesem Server läuft eine andere Backend-Version
            ({backendInfo.data?.version?.KeinPlanBackend ?? "unbekannt"}), sodass es zu
        ...
        <Stack direction="horizontal" gap={3}>
          <div className="me-auto">
            KeinPlan {frontendVersion ? `v${frontendVersion}` : "(unbekannte Version)"}
          </div>
          {backendInfo.data?.env?.GithubLink && (
            <div>
              <Link href={backendInfo.data.env.GithubLink} className="text-muted" target="_blank">
          ...
          {backendInfo.data?.env?.KaPlanLink && (
            <div>
              <Link href={backendInfo.data.env.KaPlanLink} className="text-muted" target="_blank">
